...
int main(int argc, char **argv)
{
    ...
    while (ros::ok())
    {
            ...
            if (search_result.first > 0)
            {
                ...
                gtsam::NonlinearFactor::shared_ptr near_factor(
                    new gtsam::BetweenFactor<gtsam::Pose3>(
                        cloudInd - 1, cloudInd, gtsam::Pose3(R_sam, t_sam),
                        odometryNoise));
                ...
                /*
                  ...
                  e.g. if src_key_frame_id 5 with sub frames 51~60 triggle loop
                  with tar_key_frame_id 1 with sub frames 11~20, add connection
                ...
                */
                ...
                for (size_t j = 1; j <= sub_frame_num; j++)
                {
                    ...
                    gtsam::NonlinearFactor::shared_ptr loop_factor(
                        new gtsam::BetweenFactor<gtsam::Pose3>(
                            tar_frame, src_frame, gtsam::Pose3(Rtem, ttem),
                            robustLoopNoise));
        ...
        else
        {
            ...
            {
                initial.insert(
                    0, gtsam::Pose3(gtsam::Rot3(poses_vec[cloudInd].second),
                                    gtsam::Point3(poses_vec[cloudInd].first)));
