...
int main(int argc, char **argv)
{
    ...
    while (ros::ok())
    {
        BOOST_FOREACH (rosbag::MessageInstance const m, view)
        {
            ...
            if (cloud_ptr != NULL)
            {
                    ...
                    cloudInd != 0)
                {
                    ...
                    auto t_query_end =
                        std::chrono::high_resolution_clock::now();
                    querying_time.push_back(
                        time_inc(t_query_end, t_query_begin));
                    ...
                    if (search_result.first > 0)
                    {
                        ...
                        pcl::toROSMsg(
                            *std_manager
                                 ->corner_cloud_vec_[search_result.first],
                            pub_cloud);
                        pub_cloud.header.frame_id = "camera_init";
        ...
        std::cout << "Mean time for descriptor extraction: "
                  << mean_descriptor_time << "ms, query: " << mean_query_time
