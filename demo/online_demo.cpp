...
{
    if (laser_buffer.empty() || odom_buffer.empty()) return false;
    ...
    if (abs(odom_timestamp - laser_timestamp) < 1e-3)
    {
        ...
        Eigen::Quaterniond r(odom_msg->pose.pose.orientation.w,
    ...
    {
        ROS_WARN("Current laser scan is earlier than odometry, discard one "
                 "laser scan.");
        std::unique_lock<std::mutex> l_lock(laser_mtx);
        laser_buffer.pop();
        return false;
...
{
    if (loop_container.empty()) return;
...
int main(int argc, char **argv)
{
    ...
    while (ros::ok())
    {
        ...
        if (syncPackages(current_cloud_body, pose))
        {
            ...
            pcl::transformPointCloud(*current_cloud_body, *current_cloud_world,
                                     pose);
            ...
            odom.pose.pose.position.x =
                origin_estimate_affine3d.translation().x();
            odom.pose.pose.position.y =
                origin_estimate_affine3d.translation().y();
            odom.pose.pose.position.z =
                origin_estimate_affine3d.translation().z();
            ...
            *key_cloud += *current_cloud_world;
            ...
            {
                auto prev_pose =
                    gtsam::Pose3(origin_pose_vec[cloudInd - 1].matrix());
            ...
            {
                ROS_INFO("key frame idx: [%d], key cloud size: [%d]",
                ...
                {
                    std_manager->SearchLoop(stds_vec, search_result,
                                            loop_transform, loop_std_pair);
                }
                ...
                pcl::toROSMsg(*std_manager->corner_cloud_vec_.back(),
                              pub_cloud);
                pub_cloud.header.frame_id = "camera_init";
                ...
                {
                    std::cout
                        << "[Loop Detection] triggle loop: " << keyCloudInd
                        << "--" << search_result.first
                    ...
                    // loop_transform.first.transpose() << std::endl;

                    // auto euler = loop_transform.second.eulerAngles(2, 1, 0)
                    // * 57.3; std::cout << "rotation(ypr): " << euler[0] << ' '
                    // << euler[1] << ' ' << euler[2]
                    //           << std::endl;

                    /*
                      add connection between loop frame.
                      e.g. if src_key_frame_id 5 with sub frames 51~60 triggle
                      loop with tar_key_frame_id 1 with sub frames 11~20, add
                      connection between each sub frame, 51-11, 52-12,...,60-20.
                    ...
                    */
                    ...
                    for (size_t j = 1; j <= sub_frame_num; j++)
                    {
                        ...
                        Eigen::Affine3d src_pose_refined =
                            delta_T * pose_vec[src_frame];
                    ...
                    pcl::toROSMsg(
                        *std_manager->key_cloud_vec_[search_result.first],
                        pub_cloud);
                    pub_cloud.header.frame_id = "camera_init";
            ...
            if (has_loop_flag)
            {
                ...
                for (int i = 0; i < pose_vec.size(); ++i)
                {
                    ...
                    pcl::transformPointCloud(*cloud_vec[i], correct_cloud,
                                             pose_vec[i]);
            ...
            visualizeLoopClosure(pubLoopConstraintEdge, loop_container,
                                 pose_vec);
